...
namespace Lm.Eic.App.DomainModel.Bpm.Hrm.Archives
{
    ...
    public class CalendarWeekDay
    {
        ...
        private int _weekyKey;
        public int  WeekyKey
        {
            set { _weekyKey  =value ; }
            get
            {
                if (!WeekyKeyList.Contains(_weekyKey))
                {
                    return 0;
                }
                else return _weekyKey;
            }
        }
         List<int> WeekyKeyList
        {
            get { return _date.Select(e => e.YearWeekNumber).Distinct().ToList(); }
        }
        public List<CalendarModel> WeekdayList
        {
            get
            {
                var WeekdayList = _date.Where(e => e.YearWeekNumber == WeekyKey).ToList();
                if (WeekdayList == null || WeekdayList.Count == 0) return null;
                int modelsCount = WeekdayList.Count;
                if (0 < modelsCount && modelsCount < 7)
                {
                    int yearWeek = WeekdayList.FirstOrDefault().YearWeekNumber;
                    int w = WeekdayList.FirstOrDefault().NowMonthWeekNumber ;
                    int InsertIndex = (w== 1) ? 0 : modelsCount;
                    for (int n = 1; n <= 7 - modelsCount; n++)
                    { WeekdayList.Insert(InsertIndex, new CalendarModel() { YearWeekNumber = yearWeek, CalendarDay = string.Empty }); }
                }
                WeekdayList.ForEach(e =>
                {

...
.factory('astDataopService', function (ajaxService) {
    ...
    //013935根据财产编号查询设备
    //ast.getEquipmentRepairAssetNumberDatas = function (assetNumber，formId) {
...
.controller('astInputCheckRecordCtrl', function ($scope, connDataOpService, astDataopService, $modal) {
    ...
    var vmManager = {
        init: function () {
            leeHelper.clearVM(uiVM,['CheckDate','OpSign'])
        },
        datasets: [],
        //验证是否可以保存数据
        canSave: function () {
...
.controller('astInputMaintenanceRecordCtrl', function ($scope, dataDicConfigTreeSet, connDataOpService, astDataopService, $modal) {
    ...
    var vmManager = {
        init: function () {
            leeHelper.clearVM(uiVM, ['MaintenanceDate','OpSign'])
        },
        datasets: [],
        //验证是否可以保存数据
        canSave: function () {
